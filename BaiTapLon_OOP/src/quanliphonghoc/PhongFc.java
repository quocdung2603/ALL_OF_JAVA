	...
	public void CapNhatSoMayTinh()
	{
		...
		for(int i=0;i<a.size();i++)
		{
			if(a.get(i) instanceof MayTinh && a.get(i).getMaphong().compareTo(x) == 0)
			{
				...
				// truoc convert a.get(i) : PHONG : xài dc của PHONG
				MayTinh tmp= (MayTinh) a.get(i); // convert
				// sau convert a.get(i) : MayTinh : xài dc của PHONG, MayTinh
				tmp.setSomaytinh(z);
				return;
			}
		}
